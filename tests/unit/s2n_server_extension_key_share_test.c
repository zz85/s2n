...
static int s2n_public_ecc_keys_are_equal2(struct s2n_ecc_params *params_1, struct s2n_ecc_params *params_2)
{
...
int main(int argc, char **argv)
{
    ...
    /* Test s2n_extensions_server_key_share_send_size */
    {
        ...
        EXPECT_EQUAL(( 32 * 2 ) + 1 + 8, s2n_extensions_server_key_share_send_size(conn));

        conn->secure.server_ecc_params.negotiated_curve = &s2n_ecc_supported_curves[1];
        EXPECT_EQUAL(( 48 * 2 ) + 1 + 8, s2n_extensions_server_key_share_send_size(conn));
    ...
    /* Test s2n_extensions_server_key_share_send */
    {
        ...
        uint32_t cursor;
        RECORD_WRITE_CURSOR(extension_stuffer, cursor);
        ...
        for (int i = 0; i < S2N_ECC_SUPPORTED_CURVES_COUNT; i++)
        {
            ...
            EXPECT_STUFFER_BYTES_WRITTEN(extension_stuffer, s2n_ecc_supported_curves[i].share_size + 8, cursor);
    ...
    /* Test s2n_extensions_server_key_share_recv with supported curves */
    {
        for (int i = 0; i < S2N_ECC_SUPPORTED_CURVES_COUNT; i++)
        {
            ...
            STUFFER_READ_U16_EXPECT_EQUAL(extension_stuffer, TLS_EXTENSION_KEY_SHARE);
            STUFFER_READ_U16_EXPECT_EQUAL(extension_stuffer, s2n_extensions_server_key_share_send_size(server_send_conn) - 2);

            client_recv_conn->secure.client_ecc_params[i].negotiated_curve = &s2n_ecc_supported_curves[i];
            s2n_ecc_generate_ephemeral_key(&client_recv_conn->secure.client_ecc_params[i]);
    ...
    /* Test s2n_extensions_server_key_share_recv with various payloads */
    {
        /* Test p256 key share */
        {
            ...
            client_conn->secure.client_ecc_params[0].negotiated_curve = &s2n_ecc_supported_curves[0];
            s2n_ecc_generate_ephemeral_key(&client_conn->secure.client_ecc_params[0]);

            EXPECT_SUCCESS(s2n_extensions_server_key_share_recv(client_conn, &extension_stuffer));
            EXPECT_EQUAL(client_conn->secure.server_ecc_params.negotiated_curve, &s2n_ecc_supported_curves[0]);
            EXPECT_EQUAL(s2n_stuffer_data_available(&extension_stuffer), 0);
        ...
        /* Test p384 key share */
        {
            ...
            EXPECT_NOT_NULL(client_conn = s2n_connection_new(S2N_CLIENT));
            const char *p384 = "00180061040a27264201368540483e97d324a3093e11a5862b0a1be0cf5d8510bc47ec285f5304e9ec3ba01a0c375c3b6fa4bd0ad44aae041bb776aebc7ee92462ad481fe86f8b6e3858d5c41d0f83b0404f711832a4119aec3da2eac86266f424b50aa212";

            EXPECT_NULL(client_conn->secure.server_ecc_params.negotiated_curve);
            EXPECT_SUCCESS(s2n_stuffer_alloc_ro_from_hex_string(&extension_stuffer, p384));

            client_conn->secure.client_ecc_params[1].negotiated_curve = &s2n_ecc_supported_curves[1];
            s2n_ecc_generate_ephemeral_key(&client_conn->secure.client_ecc_params[1]);

            EXPECT_SUCCESS(s2n_extensions_server_key_share_recv(client_conn, &extension_stuffer));
            EXPECT_EQUAL(client_conn->secure.server_ecc_params.negotiated_curve, &s2n_ecc_supported_curves[1]);
            EXPECT_EQUAL(s2n_stuffer_data_available(&extension_stuffer), 0);
        ...
        /* Test failure for broken p256 key share */
        {
            struct s2n_stuffer extension_stuffer;
            struct s2n_connection *client_conn;
    ...
    /* Test Shared Key Generation */
    {
        ...
        for (int i = 0; i < S2N_ECC_SUPPORTED_CURVES_COUNT; i++)
        {
            ...
            STUFFER_READ_U16_EXPECT_EQUAL(&client_hello_key_share, TLS_EXTENSION_KEY_SHARE);
            STUFFER_READ_U16_EXPECT_EQUAL(&client_hello_key_share, s2n_extensions_client_key_share_size(server_conn) - 4);
            ...
            {
                if (j != i) server_conn->secure.client_ecc_params[j].negotiated_curve = NULL;
            }
            ...
            STUFFER_READ_U16_EXPECT_EQUAL(&server_hello_key_share, TLS_EXTENSION_KEY_SHARE);
            STUFFER_READ_U16_EXPECT_EQUAL(&server_hello_key_share, s2n_extensions_server_key_share_send_size(server_conn) - 2);
            ...
            s2n_public_ecc_keys_are_equal2(&server_conn->secure.server_ecc_params, &client_conn->secure.server_ecc_params);
            s2n_public_ecc_keys_are_equal2(&server_conn->secure.client_ecc_params[i], &client_conn->secure.client_ecc_params[i]);
            ...
            EXPECT_EQUAL_S2N_BLOB(server_shared_secret, client_shared_secret);
